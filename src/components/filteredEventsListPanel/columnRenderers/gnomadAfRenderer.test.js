...
describe("GnomadAfRenderer", () => {
  ...
  test("configures gnomad_af as a numeric link without changing ClinVar", () => {
    const columns = datasets.flatMap(
      ({ optionalFilteredEventsColumns = [] }) =>
        optionalFilteredEventsColumns,
    );
    const gnomadColumn = columns.find(({ id }) => id === "gnomad_af");
    const clinvarColumn = columns.find(({ id }) => id === "clinvar");

    expect(gnomadColumn).toMatchObject({
      dataIndex: "gnomad_af",
      type: "numeric",
      viewType: "gnomad-af-link",
      filterType: "range",
      sortable: true,
      filterable: true,
      rendererProps: { format: ".3g" },
    });
