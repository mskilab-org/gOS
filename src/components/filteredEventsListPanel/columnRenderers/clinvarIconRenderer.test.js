...
describe("ClinvarIconRenderer", () => {
  ...
  test("configures only the ClinVar dataset column to use the link renderer", () => {
    const columns = datasets.flatMap(
      ({ optionalFilteredEventsColumns = [] }) =>
        optionalFilteredEventsColumns,
    );
    const clinvarColumn = columns.find(({ id }) => id === "clinvar");
    const otherAnnotationColumns = columns.filter(
      ({ id }) => ["alphamissense", "sift", "polyphen2"].includes(id),
    );

    expect(clinvarColumn.viewType).toBe("clinvar-icon");
    expect(otherAnnotationColumns).toHaveLength(3);
    otherAnnotationColumns.forEach(({ viewType }) => {
      expect(viewType).toBe("class-icon");
    });
  });
});
