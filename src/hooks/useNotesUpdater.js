 ...
 */
function formatInputDataForUpdate(
  userRequest,
  currentNotes,
  record,
  metadata,
...
${
  messageHistory.length > 0
    ? `\nCHAT HISTORY:\n${messageHistory
        .map(
          (msg) =>
            `[${msg.type === "user" ? "User" : "Bot"} at ${new Date(
              msg.timestamp
...
export function useNotesUpdater() {
  ...
  const performNotesUpdate = async (
    userRequest,
    currentNotes,
    record,
    metadata,
      ...
      for (const item of additionalContextItems) {
        if (item.type === "paper" && item.data?.pmid) {
          if (item.data.summary) {
            ...
            finalPaperSummaries[item.data.pmid] = await summarizePaper(
              item.data.fullText
            );
            ...
            finalPaperSummaries[item.data.pmid] =
              item.data.title || "No summary available";
          }
        } else if (item.type === "clinicalTrial" && item.data?.nctId) {
          finalClinicalTrials[item.data.nctId] = formatClinicalTrials(
            item.data
          );
        }
      }

      const formattedInput = formatInputDataForUpdate(
        userRequest,
        currentNotes,
        record,
        metadata,
